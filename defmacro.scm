...
;; Un expanseur de macro pour Scheme qui supporte la forme
;; "define-macro", permettant ainsi les définitions de macro dans le
;; programme source.  Les formes dérivées de Scheme, telles "let",
;; "and", et "cond" sont elles-même définies à l'aide de macros.  Dans
;; le programme source à compiler, seules les définitions de macro au
;; niveau supérieur sont permises.

;; La sortie de l'expanseur de macro est une expression qui contient
;; uniquement les formes suivantes :
;;
;; <expr> ::= <constante>
;;          | (quote <datum>)
;;          | <id>                 ;; id = nom d'une variable globale ou locale
;;          | (set! <id> <expr>)   ;; id = nom d'une variable globale ou locale
;;          | (if <expr> <expr> <expr>)
;;          | (lambda <parametres> <expr>)
;;          | (begin <def-or-expr> <expr>)
;;          | (<expr> <expr> ...)
;;
;; <def-or-expr> ::= (define <id> <expr>)   ;; id = variable globale
;;                 | <expr>
      ...
      `(let ((,var ,sujet))
         (letrec ,(append
                   (map (lambda (fn1 fn2 clause)
                        ...
                        clauses)
                   (list (list err '(lambda () (error "match failed")))))
           (,(car fns)))))))
...
;;      '(begin
;;         (define n 0)
;;         (define-macro (inc x) (list 'set! x (list '+ x 1)))
;;         (inc n)
;;         (inc n)
;;         (pp n))))
